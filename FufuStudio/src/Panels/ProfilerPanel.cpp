...
namespace FufuStudio
{
	...
	void ProfilerPanel::onImGuiRender(EditorState& state)
	{
		...
		// Triangles of the currently selected mesh, separately from the
		// scene total (useful to judge if ONE object is too heavy, not just
		// the entire scene).
		...
		// gpuSectionsMs is a per-frame map: extract "ComputePass" (and
		// "FXAAPass" if active) manually rather than via fillPlotBuffer
		// (which expects a direct field, not a map key).
