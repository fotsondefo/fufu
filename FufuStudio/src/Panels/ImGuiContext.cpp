...
namespace FufuStudio 
{
	...
	void ImGuiContext::init(void* nativeWindowHandle, const std::filesystem::path& configDir)
	{
		...
		// By default, ImGui allows moving a window by clicking and dragging
		// anywhere in its body (not just the title bar), as long as the click
		// does not land on an interactive widget. This conflicts with
		// Viewport interactions (gizmo, camera) and all other panels:
		// restrict movement to the title bar only.
	...
	{
		// All windows start at Y >= 40 (TitleBar::k_Height): the custom app
		// title bar occupies the top 40 pixels of the screen and is drawn
		// on top of everything else — a panel overlapping this strip ends up
		// partially hidden.
