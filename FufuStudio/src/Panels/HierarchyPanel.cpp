...
namespace FufuStudio
{
	...
	void HierarchyPanel::onImGuiRender(EditorState& state)
	{
		...
		// Delete key: deletes the entire current selection
	...
	void HierarchyPanel::drawEntityNode(Fufu::Entity entity, EditorState& state)
	{
		...
		if (deleted)
		{
			// If the entity is part of a multi-selection, delete the entire
			// group (as a single undo); otherwise only delete this entity.
		...
		if (ImGui::BeginPopupContextWindow("HierarchyContextMenu", ImGuiPopupFlags_MouseButtonRight | ImGuiPopupFlags_NoOpenOverItems))
		{
				...
				if (ImGui::MenuItem("Point"))
				{
					state.commandHistory->executeCommand<EntityCreateCommand>(activeScene, "Point Light", Fufu::Entity{},
						[](Fufu::Entity e)
						{
							...
							// invisible once attenuated by 1/distance²: use values
							// suited to a room lamp instead.
			...
			if (ImGui::BeginMenu("Create Primitive"))
			{
				if (ImGui::MenuItem("Cube"))
					createPrimitiveEntity(state, activeScene, "Cube", Fufu::PrimitiveMeshes::makeCube());
				if (ImGui::MenuItem("Plane"))
					createPrimitiveEntity(state, activeScene, "Plane", Fufu::PrimitiveMeshes::makePlane());
				if (ImGui::MenuItem("Sphere"))
					createPrimitiveEntity(state, activeScene, "Sphere", Fufu::PrimitiveMeshes::makeSphere());
				if (ImGui::MenuItem("Cylinder"))
					createPrimitiveEntity(state, activeScene, "Cylinder", Fufu::PrimitiveMeshes::makeCylinder());
