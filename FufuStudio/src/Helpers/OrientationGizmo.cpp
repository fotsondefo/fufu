...
namespace FufuStudio
{
	void OrientationGizmo::render(EditorState& state, Fufu::CameraController& cam)
	{
		...
		// Reconstruct the view matrix from the CameraController's pitch/yaw
		// (same convention as CameraController::onUpdate and TransformComponent).
		...
		// Draw the farthest endpoints first (simple painter's algorithm)
	...
	void OrientationGizmo::handleShortcuts(EditorState& state, Fufu::CameraController& cam)
	{
		...
		// Convention (Y-up engine): Front = +Z, Right = +X, Top = +Y — Ctrl
		// for the opposite view (Back/Left/Bottom), like Blender.
	...
	{
		// Orbit around the selected entity, or the origin if none.
		glm::vec3 pivot(0.f);
		...
		// Same derivation as the camera forward (see CameraController):
		// pitch/yaw from the desired "look" vector.
		...
		// Push immediately to the scene so the render is coherent
		// from the next frame, without waiting for onUpdate().
		auto scene = state.getActiveScene();
