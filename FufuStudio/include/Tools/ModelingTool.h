...
{
	// Face selection and extrusion on the selected entity's mesh.
	// A "face" is a triangle in the index buffer — no merging into
	// coplanar n-gons (assumed limitation for this first iteration).
	// Pick and extrude only operate on the first sub-mesh.
	...
	private:
		void extrude(EditorState& state, Fufu::Entity entity)
		{
			...
			// The extruded area is no longer the same geometry: deselect
			// to avoid accidentally extruding twice.
			m_SelectedFace.reset();
		}
