...
namespace Fufu
{
	// Simple xorshift32 PRNG: deterministic, fast, sufficient for a
	// reproducible procedural distribution by seed (not cryptographic).
		...
		std::vector<GPUTriangle>& outTriangles)
	{
		...
		// Area weights for uniform distribution over the surface
		...
		for (int s = 0; s < groom.strandCount; ++s)
		{
			...
			// Vector perpendicular to the strand, to give the ribbon its width
			...
			{
				// Downward curvature (approx. gravity in entity local space)
