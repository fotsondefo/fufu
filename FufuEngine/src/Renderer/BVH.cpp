...
namespace Fufu
{
	namespace
	{
		...
		// SAH cost of a (sub-)set of primitives grouped under `bounds`:
		// proportional to surface(bounds) * count. The usual factor of 2 from
		// surface area is omitted (constant, useless for comparing costs
		// against each other) — same principle as jbikker/pbrt.
		...
		// Generic BVH core, shared by the BLAS (triangles) and the TLAS
		// (instance bounding boxes): operates only on bounds/centroids and an
		// index permutation, without knowing the nature of the elements.
		//
		// Split by binned SAH ("fast SAH" approximation a la jbikker/pbrt):
		// tests k_SAHBins-1 candidate cut planes per axis (O(n) per axis via
		// bins, not O(n log n) via an exact sort) and keeps the cheapest of
		// the 3 axes. Much more efficient than a median split on meshes
		// with millions of triangles — a tree better balanced with respect
		// to actual geometry directly reduces the number of nodes
		// visited per ray during GPU traversal.
			...
			int leafSize)
		{
			...
			std::function<int(int, int)> buildRecursive = [&](int start, int end) -> int
			{
				...
				int   bestAxis = -1;
				float bestSplit = 0.f;
				float bestCost = sahCost(nodeBounds, count); // reference: do not split at all

				for (int axis = 0; axis < 3; ++axis)
				{
					if (extent[axis] < 1e-8f) continue; // centroids coincident on this axis
					...
					// Left-to-right then right-to-left sweep: cumulative cost
					// on each side of each cut plane in O(bins) instead of
					// recomputing AABBs for each candidate (O(bins²)).
				...
				// No cut plane improves the cost (very clustered or flat geometry):
				// make a leaf, even beyond leafSize — still correct,
				// just suboptimal for this rare case rather than forcing an
				// arbitrary split that would not help.
				if (bestAxis < 0)
				{
					makeLeaf(nodeIndex, nodeBounds, start, count);
					return nodeIndex;
				...
				// Degenerate partition (centroid duplicates exactly on the
				// boundary): fall back to a median split to ensure recursion
	...
	std::vector<GPUBVHNode> BVHBuilder::build(std::vector<GPUTriangle>& triangles, int leafSize)
	{
		...
		// Reorder triangles according to the final permutation, just once
		// (rather than shuffling ~100-byte GPUTriangle objects during recursion:
		// only ints were moved up to this point).
