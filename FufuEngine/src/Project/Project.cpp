...
namespace Fufu 
{
	...
	void Project::init()
	{
		...
		// Load ALL known scenes in the project (not just startupScene,
		// which previously only served to select the active scene
		// — the other entries in m_Info.scenes were never read).
		...
		{
			// New project, or all scene files in the manifest are missing:
			// default scene, saved immediately (and registered in the manifest)
			// so it does not disappear on next launch if the user never manually saves.
	...
	void Project::save() const
	{
		...
		j["worldSettings"]["lengthUnit"] = static_cast<int>(m_WorldSettings.lengthUnit);
	...
	std::shared_ptr<Project> Project::load(const std::filesystem::path& projFilePath)
	{
		...
		auto proj = std::make_shared<Project>(info);
		if (j.contains("worldSettings"))
			proj->m_WorldSettings.lengthUnit = static_cast<LengthUnit>(
				j["worldSettings"].value("lengthUnit", 0));
		return proj;
	}
