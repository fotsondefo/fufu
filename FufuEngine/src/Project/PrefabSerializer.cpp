...
namespace Fufu
{
	// Traverses the subtree (root included) depth-first in a deterministic
	// order: the root is always in the first position.
	...
	bool PrefabSerializer::save(Scene* scene, Entity root, const std::filesystem::path& path)
	{
		...
		doc["rootId"] = 0; // the root is always the first element (see collectSubtree)
	...
	Entity PrefabSerializer::instantiate(Scene* scene, const std::filesystem::path& path, Entity parent)
	{
		...
		// Only one version exists so far: no migration needed.
