...
namespace Fufu 
{
	...
	std::shared_ptr<Scene> SceneManager::newScene(const std::string& name)
	{
		...
		// A scene without a camera cannot display anything (see Renderer::renderScene,
		// which exits early when no primary camera is found): always start
		// with a default camera rather than leaving the user with
		// a frozen viewport showing the previous scene.
	...
	bool SceneManager::renameScene(const std::string& oldName, const std::string& newName)
	{
		...
		if (m_LoadedScenes.count(newName)) return false; // name collision
	...
	void SceneManager::setActiveScene(const std::string& name)
	{
		...
		onSceneActivated.emit(*m_ActiveScene);
	}
