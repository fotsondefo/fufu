...
namespace Fufu
{
		...
		const std::unordered_map<entt::entity, int>& indexMap)
	{
		...
		if (reg.all_of<ParentComponent>(handle))
		{
			...
			// The parent may be outside the serialized subtree (e.g. a prefab
			// whose root has a parent in the scene): in that case we simply
			// skip it, and the entity becomes a root on instantiation.
		...
		if (reg.all_of<VolumeComponent>(handle))
		{
			auto& v = reg.get<VolumeComponent>(handle);
			j["volume"] = {
				{ "albedo",           serializeVec3(v.albedo)           },
				{ "emission",         serializeVec3(v.emission)         },
				{ "density",          v.density          },
				{ "scattering",       v.scattering       },
				{ "absorption",       v.absorption       },
				{ "emissionStrength", v.emissionStrength },
				{ "anisotropy",       v.anisotropy       },
				{ "noiseType",        static_cast<int>(v.noiseType) },
				{ "noiseScale",       v.noiseScale       },
				{ "noiseOctaves",     v.noiseOctaves     },
				{ "noiseLacunarity",  v.noiseLacunarity  },
				{ "noiseGain",        v.noiseGain        },
				{ "marchSteps",       v.marchSteps       }
			};
		}

		if (reg.all_of<GaussianSplatComponent>(handle))
		{
			auto& gs = reg.get<GaussianSplatComponent>(handle);
			j["gaussianSplat"] = {
				{ "path",     gs.path     },
				{ "assetID",  gs.assetID  },
				{ "opacity",  gs.opacity  },
				{ "shDegree", gs.shDegree }
			};
		}

		if (reg.all_of<SubMeshMaterialsComponent>(handle))
		{
			auto& smm = reg.get<SubMeshMaterialsComponent>(handle);
			json slots = json::array();
			for (const auto& mat : smm.slots)
			{
				slots.push_back({
					{ "albedo",      serializeVec4(mat.albedo) },
					{ "metallic",    mat.metallic   },
	...
	Entity createEntityFromJson(Scene* scene, const json& j)
	{
		...
		// Transform already added by createEntity, we overwrite the values
		...
		if (j.contains("light"))
		{
			...
			l.unit = static_cast<LightUnit>(j["light"].value("unit", 0));
		...
		{
			VolumeComponent v;
			const auto& jv = j["volume"];
			if (jv.contains("albedo"))   v.albedo   = deserializeVec3(jv["albedo"]);
			if (jv.contains("emission")) v.emission = deserializeVec3(jv["emission"]);
			v.density          = jv.value("density",          1.f);
			v.scattering       = jv.value("scattering",       0.5f);
			v.absorption       = jv.value("absorption",       0.1f);
			v.emissionStrength = jv.value("emissionStrength", 0.f);
			v.anisotropy       = jv.value("anisotropy",       0.f);
			v.noiseType        = static_cast<VolumeNoiseType>(jv.value("noiseType", 0));
			v.noiseScale       = jv.value("noiseScale",       4.f);
			v.noiseOctaves     = jv.value("noiseOctaves",     4);
			v.noiseLacunarity  = jv.value("noiseLacunarity",  2.f);
			v.noiseGain        = jv.value("noiseGain",        0.5f);
			v.marchSteps       = jv.value("marchSteps",       64);
			entity.addComponent<VolumeComponent>(v);
		}

		if (j.contains("gaussianSplat"))
		{
			GaussianSplatComponent gs;
			gs.path     = j["gaussianSplat"].value("path",     std::string{});
			gs.assetID  = j["gaussianSplat"].value("assetID",  uint64_t(0));
			gs.opacity  = j["gaussianSplat"].value("opacity",  1.f);
			gs.shDegree = j["gaussianSplat"].value("shDegree", 3);
			entity.addComponent<GaussianSplatComponent>(gs);
		}

		if (j.contains("subMeshMaterials") && j["subMeshMaterials"].is_array())
		{
			SubMeshMaterialsComponent smm;
			for (const auto& s : j["subMeshMaterials"])
			{
				MaterialComponent mat;
				mat.albedo      = deserializeVec4(s["albedo"]);
				mat.metallic    = s.value("metallic",    0.f);
				mat.roughness   = s.value("roughness",   1.f);
				mat.emissive    = s.value("emissive",    0.f);
				mat.ior         = s.value("ior",         1.5f);
				mat.albedoTexID = s.value("albedoTexID", uint64_t(0));
				mat.normalTexID = s.value("normalTexID", uint64_t(0));
				mat.ormTexID    = s.value("ormTexID",    uint64_t(0));
				smm.slots.push_back(mat);
			}
			entity.addComponent<SubMeshMaterialsComponent>(smm);
		}
