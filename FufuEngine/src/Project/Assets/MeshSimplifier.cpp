...
namespace Fufu
{
	...
	SubMesh MeshSimplifier::simplify(const SubMesh& source, float cellSize)
	{
		...
		for (std::size_t i = 0; i + 2 < source.indices.size(); i += 3)
		{
			...
			if (a == b || b == c || a == c)
				continue; // degenerate triangle after vertex merging
		...
		// Safety net: if clustering has made everything degenerate (cellSize too
		// large for this mesh), fall back to the original geometry rather than
