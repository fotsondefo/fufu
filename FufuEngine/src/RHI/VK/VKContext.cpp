...
void VKContext::createInstance(bool validation)
{
    ...
    // Extensions required by GLFW + debug
...
void VKContext::createDevice()
{
    ...
    // Vulkan 1.2 features: descriptor indexing (update-after-bind)
...
void VKContext::destroyFrameResources()
{
    for (uint32_t i = 0; i < kFramesInFlight; ++i)
    {
        ...
        // commandBuffer belongs to the pool — no individual free needed
    }
}
