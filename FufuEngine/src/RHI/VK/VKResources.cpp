// FUFU_VK_BACKEND defined for this TU → depch.h does not include glad
...
static std::string loadVKShaderSource(const std::string& sourcePath)
{
    ...
    // Look first in shaders/vulkan/<name>, then shaders/<name>
                                              ...
                                              ShaderStage stage)
{
    ...
    opts.disableOptimizer = true;  // ENABLE_OPT=OFF → no SPIRV-Tools
    ...
    : m_Device(device)
{
    ...
    {
        // Pre-compiled SPIR-V provided directly
        spirv.resize(desc.spirvCode.size() / 4);
        std::memcpy(spirv.data(), desc.spirvCode.data(), desc.spirvCode.size());
    }
    else
    {
        // Load the Vulkan variant from shaders/vulkan/ (or compile glslSource)
