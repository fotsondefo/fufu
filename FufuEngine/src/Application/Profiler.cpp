...
namespace Fufu
{
	...
	{
		// Iterates pending queries and retrieves those already ready,
		// WITHOUT ever waiting (GL_QUERY_RESULT_AVAILABLE) — otherwise we stall
		// the GPU pipeline, which is exactly what multi-buffering avoids.
	...
	void Profiler::endFrame()
	{
		...
		// FIFO ring buffer: O(n) cost per frame on erase(begin()), but
		// n <= k_HistorySize (240) — negligible once per frame.
