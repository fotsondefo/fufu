...
struct ContextDesc
{
    ...
    bool     enableValidation = false; // VK validation layers / GL debug callback
...
public:
    ...
    // Instantiates GLContext (OpenGL) or VKContext (Vulkan) based on desc.backend.
    ...
    // ── Frame ─────────────────────────────────────────────────────────────────
    // beginFrame() returns the CommandList ready to record.
    // A single active CommandList per frame (single-threaded recording).
    virtual RHICommandList* beginFrame() = 0;
    virtual void            endFrame()   = 0; // submit + synchronization
